...
enum BookAIPrompts {
    ...
    Erkläre den Inhalt dieser Seite verständlich. Stelle zuerst die Kernaussage heraus.
    Erkläre danach die Zusammenhänge und definiere Fachbegriffe knapp.
    """

    static let summarizePage = """
    Fasse diese Seite knapp zusammen. Nenne nur die wichtigsten Aussagen.
    Hebe Begriffe oder Ergebnisse hervor, die man sich merken sollte.
    """

    static let explainFigure = """
    Untersuche die wichtigste Abbildung, Karte, Tabelle oder das wichtigste Diagramm auf dieser Seite.
    Erkläre zuerst, was dargestellt wird, dann wie man es liest und zuletzt, welche Aussage daraus folgt.
    Falls keine Abbildung erkennbar ist, sage das klar.
    """

    static func solveExercise(_ number: String) -> String {
        """
        Löse Aufgabe \(number). Gib zuerst das Ergebnis an.
        Zeige danach den vollständigen Lösungsweg in nachvollziehbaren, nummerierten Schritten.
        Begründe jeden nicht offensichtlichen Schritt und prüfe das Ergebnis, wenn das möglich ist.
        Falls die Aufgabe oder notwendige Angaben auf den ausgewählten Seiten nicht eindeutig erkennbar sind,
        benenne genau, was fehlt, statt etwas zu erfinden.
        """
    }

    static let simplerFollowUp =
        "Erkläre die vorige Antwort noch einmal einfacher, ohne wichtige Bedingungen wegzulassen."
    ...
    static let shorterFollowUp = """
    Fasse die vorige Antwort kürzer und einfacher zusammen.
    Behalte das Ergebnis und alle notwendigen Einschränkungen bei.
    """
    ...
    static func formatted(_ request: String) -> String {
        ...
        - Hebe das Endergebnis und zentrale Begriffe **fett** hervor.
          Verwende nummerierte Listen für Lösungswege und Aufzählungen nur für echte Listen.
        - Setze alleinstehende Formeln in einen Markdown-Codeblock mit der Sprache `math`.
          Erkläre anschließend alle Variablen. Verwende keine Tabellen und keine unnötige Einleitung.
